...
文章索引按页拆分存放，文件名形如 `index_1.json`、`index_2.json`，每个文件代表一页（每页最多 20 篇）。

- **编号越大的文件代表越新的内容**（`index_1.json` 为最旧一页）
- 首页展示时**从最大的编号开始**，即最新一页排在最前
- 每个分页内按日期降序排列，确保最新在前

添加新文章时，将记录放入当前编号最大的索引文件末尾（保持按日期升序书写，展示时会自动按日期降序）；若该页已满 20 条，则创建新的 `index_{N+1}.json`。
...
> **注意**：新建分页文件（如 `index_2.json`）后，还需在 `app/server/routes/sitemap.xml.ts` 中手动添加对应的静态导入并合并（服务端构建不支持自动 glob），否则新分页不会出现在 sitemap 中。
...
- **编号越大的文件代表越新的内容**（`page-1.json` 为最旧一页）
- 展示时**从文件名数字大的开始作为第一页**，即最新一页排在最前
- 页内按 `id` 从大到小排列：`id` 越大表示越后写的（越新），显示在最前
- 添加新说说时放入当前编号最大的文件末尾，`id` 取现有最大 `id` 加一；若该页已满 20 条，创建新的 `page-{N+1}.json`
...
├── content/
│   ├── posts/
│   │   ├── index_1.json          # 文章索引分页（每页 20 篇，编号越大越新）
│   │   ├── zh/                   # 中文文章
│   │   │   ├── hello-world.json
│   │   │   ├── life-thoughts.json
│   │   │   ├── nuxt3-guide.json
